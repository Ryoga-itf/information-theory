...
通信路の容量 $C$ は以下の式により計算することができる。

$
C = 1 - H_2(p)
$

ここで、$H_2(p)$ は二元エントロピー関数で、
...
本課題においては $p = 0.01$ であるから、これを用いて計算すると、$C approx 0.919$ である。

すべての $m$ に対して $R < C$ であるから、通信符号化定理により任意に小さい復号誤り率で送信できる通信路符号が存在することがわかる。
よって、理論的には大きな改善の余地があると言える。
同じ情報伝送速度 $R$ でより良い誤り訂正符号を用いることで、これよりも誤り率を下げられると考えられる。
ハミング符号よりも優れた方法を用いれば改善の余地が十分にあると言える。
